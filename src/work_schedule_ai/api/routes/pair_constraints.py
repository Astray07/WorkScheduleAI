...
@router.get(
    ...
    response_model=list[PairConstraintResponse],
)
def list_pair_constraints(
    organization_id: str,
    ...
    pair_constraints = db_session.execute(
        ...
        .where(PairConstraint.organization_id == organization_id)
        .order_by(PairConstraint.normalized_employee_a_id, PairConstraint.id)
    ).scalars()
    return [
        _pair_constraint_response(pair_constraint)
...
@router.patch(
    "/{organization_id}/pair-constraints/{pair_constraint_id}",
    response_model=PairConstraintResponse,
)
def update_pair_constraint(
    organization_id: str,
    ...
    if pair_constraint is None or pair_constraint.organization_id != organization_id:
        raise HTTPException(status_code=404, detail="Pair constraint not found")

    normalized_a, normalized_b = _normalize_pair_or_422(request)
    _validate_pair_employees_or_422(organization_id, request, db_session)
    duplicate = db_session.execute(
        select(PairConstraint).where(
            PairConstraint.organization_id == organization_id,
            PairConstraint.normalized_employee_a_id == normalized_a,
            PairConstraint.normalized_employee_b_id == normalized_b,
            PairConstraint.type == request.type,
            PairConstraint.id != pair_constraint_id,
        )
    ).scalar_one_or_none()
    if duplicate is not None:
        raise HTTPException(
    ...
    pair_constraint.employee_a_id = request.employee_a_id
    pair_constraint.employee_b_id = request.employee_b_id
    pair_constraint.normalized_employee_a_id = normalized_a
    pair_constraint.normalized_employee_b_id = normalized_b
    pair_constraint.type = request.type
    pair_constraint.severity = request.severity
    pair_constraint.override_allowed = request.override_allowed
    pair_constraint.active = request.active
    db_session.commit()
    return _pair_constraint_response(pair_constraint)
...
def _normalize_pair_or_422(
    request: PairConstraintCreateRequest,
) -> tuple[str, str]:
    try:
        return normalize_pair_employee_ids(request.employee_a_id, request.employee_b_id)
    except ValueError as exc:
        raise HTTPException(
            status_code=422,
            detail={
...
def _validate_pair_employees_or_422(
    organization_id: str,
    request: PairConstraintCreateRequest,
    db_session: Session,
) -> None:
