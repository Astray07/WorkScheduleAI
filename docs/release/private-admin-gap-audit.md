...
- cross-tenant DB hardening으로 `employee_roles`, `unavailabilities`, `employee_user_links`에 tenant composite FK를 추가했습니다. 전체 테이블 확장 순서는 `docs/release/cross-tenant-constraint-plan.md`에 문서화되어 있습니다.
